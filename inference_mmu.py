...
def run_mmu(config, model):
    ...
    for i in tqdm(range(config.samples_count)):
        ...
        pixel_values = clip_image_processor.preprocess(image_ori, return_tensors="pt")[
            "pixel_values"
        ][0]
        ...
        if config.model.showo.w_clip_vit:
            ...
            input_ids = torch.nn.utils.rnn.pad_sequence(
                input_ids,
                batch_first=True,
                padding_value=uni_prompting.text_tokenizer.pad_token_id,
            )
