...
def train_step(
    ...
    input_ids_t2i, labels_t2i, mask_prob, image_tokens_ori = prepare_inputs_and_labels(
        pixel_values, texts, vq_model, uni_prompting, mask_id, config, 
        mask_schedule, config.training.min_masking_rate)
    ...
    input_ids_lm, _, labels_lm = uni_prompting((texts_lm, input_ids_t2i.shape[-1]), 'lm')
    ...
    # Log metrics
    if accelerator.sync_gradients and (global_step + 1) % config.experiment.log_every == 0:
...
def main():
    ...
    if config.dataset.gen_type == "t2i_parquet" or config.dataset.und_type == "captioning_parquet":
        try:
            from datasets import create_imagetext_dataloader as create_imagetext_dataloader_fn
        except ImportError:
            logger.warning("create_imagetext_dataloader not available, but required for parquet mode")
    
    combined_dataloader, num_update_steps_per_epoch, num_train_epochs = create_dataloaders(
        config=config,
        accelerator=accelerator,
        tokenizer=tokenizer,
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for batch, batch_idx, dataloader_idx in combined_dataloader:
            ...
            if accelerator.sync_gradients:
                ...
                # Генерируем на первом шаге и затем каждые generate_every шагов
                if ((global_step + 1) == 1 or (global_step + 1) % config.experiment.generate_every == 0) and accelerator.is_main_process:
                    generate_images(
                        model,
                        vq_model,
                        uni_prompting,
